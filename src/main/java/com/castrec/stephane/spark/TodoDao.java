...
public class TodoDao {
  ...
  public static List<Todo> ofStatus(String statusString) {
    return (statusString == null || statusString.isEmpty()) ? DATA : ofStatus(Todo.Status.valueOf(statusString.toUpperCase()));
  }

  public static List<Todo> ofStatus(Todo.Status status) {
  ...
  public static void removeCompleted() {
    ofStatus(Todo.Status.COMPLETE).forEach(t -> TodoDao.remove(t.getId()));
  }
  ...
  public static void toggleAll(boolean complete) {
    TodoDao.all().forEach(t -> t.setStatus(complete ? Todo.Status.COMPLETE : Todo.Status.ACTIVE));
  }
